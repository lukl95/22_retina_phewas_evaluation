...
def load_data():
    ...
    out_path = f"{experiment_path}/benchmarks_cvd"
    pathlib.Path(out_path).mkdir(parents=True, exist_ok=True)

    prediction_paths = pd.read_feather(f"{experiment_path}/prediction_paths_CVD.feather")
...
#     rows = [ray.get(r) for r in tqdm(rows_ray)] # ray
    ...
    pathlib.Path(f"{experiment_path}/benchmarks_cvd/{today}").mkdir(parents=True, exist_ok=True)
    
    benchmark_endpoints.to_feather(f"{experiment_path}/benchmarks_cvd/{today}/{name}.feather")
