...
def fit_cox(data_fit, feature_set, covariates, endpoint, penalizer, step_size=1):
    ...
    data_fit = data_fit.select_dtypes(include=['int', 'float32']).loc[:, data_fit.select_dtypes(include=['int', 'float32']).nunique() > 1]
...
def clean_covariates(endpoint, covariates):
    ...
    if endpoint=="phecode_705-1": # Rheumatoid arthritis and other inflammatory
        covariates = [c for c in covariates if c not in ['rheumatoid_arthritis']]
    if endpoint=='phecode_144':
        covariates = [c for c in covariates if c not in ['sex_Male', 'male_erectile_dysfunction']]
    if endpoint=='phecode_610-1':
        covariates = [c for c in covariates if c not in ['sex_Male', 'male_erectile_dysfunction']]
    ...
    if endpoint=='phecode_627-4':
        covariates = [c for c in covariates if c not in ['sex_Male', 'male_erectile_dysfunction']]
    #if endpoint=='phecode_610-1':
    #    covariates = [c for c in covariates if c not in ['corticosteroids', 'antihypertensives', 'sex_Male', 'systemic_lupus_erythematosus', 'schizophrenia', 'bipolar_disorder']]
    #'ethnic_background_Chinese'
    #'corticosteroids'
    #'renal_failure'
    return covariates
